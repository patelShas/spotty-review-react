...
export const findReviewsByAlbum = async (album_id) => {
    const response = await axios.get(`${REVIEWS_API}?album_id=${album_id}`);
    const reviews = response.data;
    return reviews;
}

export const findReviewsByUser = async (user_name) => {
    const response = await axios.get(`${REVIEWS_API}?reviewer=${user_name}`);
    const reviews = response.data;
    return reviews;
}
